...
class GaussianDiffusion_Nolatent(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, cond=None, sphere=None, cond_scale=1.):
        noise = self.denoise_fn.forward_with_cond_scale(x, t, cond=cond, sphere=sphere, cond_scale=cond_scale)
    ...
    def p_sample(self, x, t, cond=None, cond_scale=1., clip_denoised=True, sphere=None):
        b, *_, = x.shape
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, t=t, clip_denoised=clip_denoised, cond=cond, cond_scale=cond_scale, sphere=sphere)
    ...
    @torch.inference_mode()
    def p_sample_loop(self, shape, cond=None, cond_scale=1., sphere=None):
        device = self.betas.device
        b = shape[0]
        img = torch.randn(shape, device=device)
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, torch.full(
                (b,), i, device=device, dtype=torch.long), cond=cond, cond_scale=cond_scale, sphere=sphere)
        return img

    def undo(self, image_before_step, img_after_model, t, debug=False):
    ...
    def p_losses(self, x_start, t, sphere, cond=None, noise=None, **kwargs):
    ...
    def forward(self, x, sphere, *args, **kwargs):
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            for _ in range(self.gradient_accumulate_every):
                ...
                sphere = data_frame['sphere'].to(self.device)
